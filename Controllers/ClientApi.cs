                ...
                foreach (var company in allCompanies)
                {
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(nextCSDateStr))
                        {
                            ...
                            {
                                company.CS01D = formattedDate;
                                hasChanges = true;
                            }
                        }

                        // Handle identity verification due date comparison (Next Due)
                        string nextDueStr = (companyHouseData.identity_verification?.next_due != null) 
                            ? (string)companyHouseData.identity_verification.next_due 
                            : (string)companyHouseData.confirmation_statement?.next_due;
                            
                        if (!string.IsNullOrEmpty(nextDueStr))
                        {
                            DateTime nextDueDate = DateTime.Parse(nextDueStr);
                            string formattedDate = nextDueDate.ToString("yyyy-MM-dd");

                            if (company.IdentityVerificationD != formattedDate)
                            {
                                company.IdentityVerificationD = formattedDate;
                                hasChanges = true;
                            }
                        }
